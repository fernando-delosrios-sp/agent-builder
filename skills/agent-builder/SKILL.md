...
**CRITICAL — UPSTREAM SKILL OVERRIDES:**
...
- ALWAYS start from `templates/`. Copy ALL template files into `agents/<agent-name>/` first, then populate. Never write agent files from scratch or invent new file structures.

**Template Files Reference:**
The template produces these exact files in `agents/<agent-name>/`:

| Template source | Output file | Action |
|---|---|---|
| `templates/AGENTS.md` | `AGENTS.md` | Populate with agent-specific mandates, workflow, constraints |
| `templates/docs/IDENTITY.md` | `docs/IDENTITY.md` | Fill `{{ placeholders }}` from Requirements Brief |
| `templates/docs/CONTEXT.md` | `docs/CONTEXT.md` | Fill with domain terms resolved during the Grill |
| `templates/docs/JOURNAL.md` | `docs/JOURNAL.md` | Copy as-is |
| `templates/docs/MINDSET.md` | `docs/MINDSET.md` | Copy as-is |
| `templates/docs/PROGRESS.md` | `docs/PROGRESS.md` | Copy as-is |
| `templates/docs/RULES.md` | `docs/RULES.md` | Copy as-is |
| `templates/.gitignore` | `.gitignore` | Copy as-is |
...
- `docs/IDENTITY.md` — filled with agent identity details from the Requirements Brief.
- `docs/CONTEXT.md` — populated with domain terms resolved during the Grill (Step 1). The resulting agent MUST maintain this file via `grill-with-docs` (challenge against glossary, sharpen fuzzy language, update inline).
- `AGENTS.md` — updated with agent-specific mandates, session protocol, and constraints. Must include a mandate that `docs/CONTEXT.md` is maintained via `grill-with-docs`.

The template's `docs/JOURNAL.md`, `docs/MINDSET.md`, `docs/PROGRESS.md`, and `docs/RULES.md` are carried through as-is into the output bundle.
...
- [ ] Sub-agents are at `agents/<parent>/agents/<sub>/` or degit-deployed independently to `agents/<sub>/`
